    ...
    <div className="space-y-4 sm:space-y-5">
      ...
      <div className="flex items-center gap-3 bg-[#111122] border border-white/10 rounded-xl px-4 py-3">
        ...
        <input
          ...
          className="bg-transparent text-white text-sm sm:text-base placeholder-zinc-500 outline-none flex-1 min-w-0"
        />
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
        <div className="bg-[#111122] border border-white/5 rounded-2xl p-4">
          <p className="text-zinc-500 text-xs sm:text-sm mb-1">Total bookings</p>
          <p className="text-white text-2xl sm:text-3xl font-bold">{attendees.length}</p>
        </div>
        <div className="bg-[#111122] border border-white/5 rounded-2xl p-4">
          <p className="text-zinc-500 text-xs sm:text-sm mb-1">Validated</p>
        ...
        <div className="bg-[#111122] border border-white/5 rounded-2xl p-4 col-span-2 sm:col-span-1">
          <p className="text-zinc-500 text-xs sm:text-sm mb-1">Pending</p>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="hidden md:block bg-[#111122] border border-white/5 rounded-2xl overflow-hidden">
            <div className="grid grid-cols-12 gap-4 px-5 py-3 border-b border-white/5 text-zinc-500 text-sm font-medium">
              ...
              <div key={ticket.id} className="p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-zinc-500 text-xs truncate">{ticket.guestEmail || ticket.user?.email || '—'}</p>
                  </div>
      ...
      <div className="min-h-screen bg-[#090912] text-white flex flex-col">
        ...
        <main className="relative max-w-3xl w-full mx-auto px-4 sm:px-6 pt-24 sm:pt-28 pb-16 sm:pb-20 flex-1">
          <button
            ...
            className="flex items-center gap-2 text-zinc-400 hover:text-white transition-colors mb-6 sm:mb-8 group text-sm"
          >
          ...
          <div className={`rounded-2xl p-5 sm:p-8 border text-center ${isPending ? 'bg-yellow-500/5 border-yellow-500/20' : isRejected ? 'bg-red-500/5 border-red-500/20' : 'bg-white/5 border-white/10'}`}>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-4 text-xs sm:text-sm text-zinc-500 mb-8">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-3">
              {event.status === 'DRAFT' && (
                <button
                  ...
                  className="bg-[#6c47ff] hover:bg-[#7c57ff] text-white text-sm font-medium px-6 py-3 rounded-xl transition-all w-full sm:w-auto"
                >
                  Submit for review
                </button>
              )}
              <button
                onClick={() => navigate('/org/dashboard')}
                className="bg-white/5 border border-white/10 hover:border-white/20 text-white text-sm font-medium px-6 py-3 rounded-xl transition-all w-full sm:w-auto"
              >
                Back to dashboard
              </button>
            </div>
      ...
      <main className="relative max-w-5xl w-full mx-auto px-4 sm:px-6 pt-24 sm:pt-28 pb-16 sm:pb-20 flex-1">
        ...
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-5 mb-7 sm:mb-8">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-zinc-500 text-sm leading-relaxed break-words">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            <div
              className={`flex items-center gap-2 text-xs sm:text-sm font-medium px-3 sm:px-4 py-2.5 rounded-xl ${
                event.status === 'PUBLISHED'
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 text-xs sm:text-sm font-medium px-3 sm:px-4 py-2.5 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20 transition-all"
            >
              <Trash2 size={14} />
              <span>Delete</span>
            </button>
          ...
          {['overview', 'validators', 'attendees'].map((item) => (
            <button
              ...
              className={`px-3 sm:px-4 py-3 text-sm sm:text-base font-medium capitalize transition-colors border-b-2 -mb-px whitespace-nowrap ${
                tab === item ? 'border-[#6c47ff] text-white' : 'border-transparent text-zinc-500 hover:text-zinc-300'
              }`}
            >
              {item}
            </button>
          ...
          <div className="space-y-4 sm:space-y-5">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              <div className="bg-[#111122] border border-white/5 rounded-2xl p-4 sm:p-5">
                <p className="text-zinc-500 text-xs sm:text-sm mb-2">Tickets booked</p>
                <p className="text-white text-2xl sm:text-3xl font-bold">{ticketCount}</p>
              </div>
              <div className="bg-[#111122] border border-white/5 rounded-2xl p-4 sm:p-5">
                <p className="text-zinc-500 text-xs sm:text-sm mb-2">Capacity</p>
                <p className="text-white text-2xl sm:text-3xl font-bold">{event.capacity}</p>
              </div>
              <div className="bg-[#111122] border border-white/5 rounded-2xl p-4 sm:p-5">
                <p className="text-zinc-500 text-xs sm:text-sm mb-2">Validators</p>
                <p className="text-white text-2xl sm:text-3xl font-bold">{validators.length}</p>
              </div>
              <div className="bg-[#111122] border border-white/5 rounded-2xl p-4 sm:p-5">
                <p className="text-zinc-500 text-xs sm:text-sm mb-2">Spots left</p>
            ...
            <div className="bg-[#111122] border border-white/5 rounded-2xl p-5 sm:p-6">
              <div className="flex items-start justify-between gap-4 mb-4">
                <div className="min-w-0">
                  ...
                  <p className="text-zinc-500 text-xs sm:text-sm">Share this PIN with your gate validators</p>
                </div>
                <button
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="flex-1 min-w-0 bg-white/5 border border-white/10 rounded-xl px-3 sm:px-5 py-3 sm:py-4 font-mono text-lg sm:text-2xl tracking-[0.25em] sm:tracking-[0.5em] text-white overflow-hidden">
                  <span className="block truncate">{showPin ? event.eventPin : '••••••'}</span>
                </div>
                <button
                  onClick={handleCopyPin}
                  className="flex items-center justify-center bg-[#6c47ff]/20 border border-[#6c47ff]/30 hover:bg-[#6c47ff]/30 text-[#a78bfa] w-12 h-12 sm:w-auto sm:h-auto sm:px-4 sm:py-4 rounded-xl transition-all shrink-0"
                  aria-label="Copy PIN"
                >
                  <Copy size={16} />
                </button>
          ...
          <div className="space-y-4 sm:space-y-5">
            <div className="bg-[#111122] border border-white/10 rounded-2xl p-5 sm:p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white text-sm sm:text-base placeholder-zinc-500 outline-none focus:border-[#6c47ff]/50 transition-colors"
                />
                <input
                  type="email"
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 w-full sm:w-auto bg-[#6c47ff] hover:bg-[#7c57ff] disabled:opacity-50 text-white text-sm sm:text-base font-medium px-5 py-3 rounded-xl transition-all"
              >
                {addingValidator ? (
            ...
            {validators.length === 0 ? (
              <div className="bg-[#111122] border border-white/5 rounded-2xl p-8 sm:p-10 text-center">
                <Users size={40} className="text-zinc-700 mx-auto mb-3" />
                <p className="text-zinc-500 text-sm sm:text-base">No validators added yet</p>
              </div>
            ) : (
              <div className="space-y-3">
                {validators.map((validator, index) => (
                  <div key={validator.id} className="bg-[#111122] border border-white/5 rounded-2xl p-4 sm:p-5 flex items-center gap-3 sm:gap-4 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-zinc-500 text-xs sm:text-sm truncate mt-0.5">{validator.email}</p>
                    </div>
