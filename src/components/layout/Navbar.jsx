...
import { Bell, User, LogOut, Ticket, CheckCheck, X, Menu } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            ...
            >
              {link.label}
              <span
                className={`absolute -bottom-1 left-0 h-0.5 bg-[#6c47ff] transition-all duration-200 ${
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          {user ? (
            <>
              ...
              <div className="relative" ref={notifRef}>
                <button
                  ...
                  className="relative text-zinc-400 hover:text-white transition-colors p-2 rounded-xl hover:bg-white/5"
                    ...
                    <div className="max-h-80 overflow-y-auto">
                      {notifications.length === 0 ? (
                        ...
                        notifications.map((n) => (
                          ...
                          >
                            <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5 ${!n.read ? 'bg-[#6c47ff]/20' : 'bg-white/5'}`}>
                              <Ticket size={14} className={!n.read ? 'text-[#a78bfa]' : 'text-zinc-500'} />
                            </div>
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-zinc-500 text-[11px] mt-1">{timeAgo(n.createdAt)}</p>
                            </div>
                            {!n.read && <div className="w-2 h-2 rounded-full bg-[#6c47ff] flex-shrink-0 mt-2" />}
                            <button
                              onClick={(e) => handleDelete(e, n.id)}
                              className="text-zinc-500 hover:text-zinc-300 transition-colors flex-shrink-0 mt-0.5 p-1"
              ...
              <div className="relative" ref={menuRef}>
                ...
                >
                  ...
                  <span className="text-sm text-zinc-300 hidden md:block">{user.name?.split(' ')[0]}</span>
                </button>
            ...
            <Link
              to="/login"
              className="bg-[#6c47ff] hover:bg-[#7c57ff] text-white text-sm font-semibold px-4 py-2 rounded-xl transition-all shadow-md shadow-[#6c47ff]/20"
            >
              Sign in
            </Link>
          )}
